...
- **시크릿은 `.env`에서 읽기만** (호스팅·디자인·트래커 등 *이 배포가 실제로 쓰는 통합*의 토큰 — 어떤 것인지는 배포마다 다르다. 특정 벤더를 전제하지 않는다). 추적 파일(dlc-meta 포함)에 값 커밋 금지. 토큰 발급·접근제어 변경은 사람 (D-Stage4-2).
...
- **POLICY-ENCODING (파일 인코딩·줄바꿈)** — 생성·구체화(materialize)되는 *모든* 파일은 **UTF-8 (BOM 없음)** 으로 기록한다(하드 규칙). **줄바꿈(EOL)은 대상 레포의 컨벤션을 따른다** — `.gitattributes`·`.editorconfig`·기존 파일에서 탐지하고, 단서가 없으면 LF를 기본으로 한다(범용 툴이므로 CRLF 컨벤션 레포도 존중). **로케일 의존 셸 출력(echo·리다이렉트·tee·기본 `Set-Content` 등)으로 비ASCII 내용을 절대 내보내지 말 것** — 일부 OS 콘솔(예: Windows CP949)이 멀티바이트 텍스트를 깨뜨린다(실제 손상 사례의 근인 — 이 방지가 본 정책의 핵심). 반드시 *직접 UTF-8 파일 쓰기*를 사용한다. 생성 후 각 파일을 **검증** — U+FFFD(치환문자)·비ASCII 영역의 떠도는 `?`·BOM이 있으면 손상으로 보고 재생성한다.
  - **판정·검사 명령의 *출력*도 UTF-8로 고정한다 (같은 정책의 반대 방향).** 파일 쓰기만 지키고 출력을 방치하면, 위반한 줄(em dash `—`·한국어 등)을 *출력하려는 순간* 비-UTF-8 콘솔이 `UnicodeEncodeError`를 내고 명령이 죽는다 — **위반이 없을 때만 조용히 통과하고 위반이 있을 때만 죽는** 최악의 실패 방향이며, 검증 도구가 검증해야 할 순간에만 무력해진다(실측된 결함). 파이썬 판정 명령은 첫 구문을 `import sys;sys.stdout.reconfigure(encoding='utf-8', errors='backslashreplace');` 로 고정한다(3.6 이하는 `PYTHONIOENCODING=utf-8` 로 대체하고 무엇을 붙였는지 보고에 명시). 다른 언어·수단을 쓰면 *동치의 출력 인코딩 고정*을 적용한다.
- **POLICY-TEMPLATE-ADHERENCE (템플릿 준수)** — 모든 "인스턴스" 산출물은 반드시 대응하는 `templates/*.template.md`에서 **생성**한다 (같은 경로 규약·섹션·변수 치환 유지). 템플릿이 있는 산출물을 자유 형식으로 손수 작성하는 것은 *결함*이다. 각 생성기는 자신의 *템플릿 → 인스턴스 경로* 매핑을 명시해야 한다. **템플릿 안에서 무엇이 "생성 시 사라져야 할 저작 지시"이고 무엇이 "인스턴스가 계속 지녀야 할 안내"인지, 그리고 생성 후 `{...}` 잔존 검사를 어떻게 판정하는지는 아래 §3.2가 정본이다.**
...
- **POLICY-ISSUE-TRACKING (사이클↔티켓 바인딩, 조건부)** — 시스템이 이슈 트래커를 운영할 때만 발동하는 *조건부* 규율. 오케스트레이터는 각 작업 사이클을 티켓에 바인딩하고(범위 안: 사이클=티켓, START 생성+In-Progress 전이 / CLOSE Done 전이) 범위 밖 작업을 트리아지 요청 티켓으로 발행한다. 트래커 종류·베이스 URL·프로젝트 키·인증·상태명은 *인스턴스 config에서* 오고 공유 룰북에 박지 않는다(적응형); API 토큰은 시크릿 스토어(`.env`/토큰 파일 경로)에만. 규율 정본은 `specs/ISSUE-TRACKER-ADAPTER.md`, 실행 주체는 `agents/ISSUE-TRACKER-AGENT.md`. Jira가 첫 어댑터.

**현재 단계 명시 컨벤션** (원본 보존): 오케스트레이터는 사용자 보고 시 현재 STEP을 머리에 명시. 예: `[STEP 4 — 레포 AWS 위탁 진행 중]`. 다중 레포 병렬 시 `[STEP 4 — repo:api / repo:frontend 병렬]`.

### 3.2 템플릿 표기 규약 & `{...}` 잔존 판정 (POLICY-TEMPLATE-ADHERENCE 부속 — 정본)

> **왜 있나**: 템플릿의 *설명용 산문*이 인스턴스로 그대로 전사되면서, 그 산문 안의 리터럴 `{VAR}`가 생성 후 "미충전 변수 잔존" 검사에 **거짓 양성**을 만든 실측 결함이 있다. 문제는 두 축이다 — ① 템플릿이 *지시*와 *안내*를 구분하지 않는다 ② 검사가 *무엇이 위반인지*를 정의하지 않아 실행자가 매번 지어낸다. 아래가 두 축의 정본이며, **모든 `templates/*.template.md`와 모든 생성기(SETTER·REPO-SETTER·HANDOFF-WRITER·CYCLE-CLOSER 등)에 예외 없이 적용**된다.

**(1) 템플릿 안의 세 가지 텍스트 — 구분해서 쓴다**

| 종류 | 표기 | 생성 시 | 예 |
|---|---|---|---|
| **A. 충전 슬롯** (fill slot) | `{UPPER_SNAKE}` — **ASCII 대문자로 시작하는 대문자·숫자·밑줄만** (`{SYSTEM_NAME}`·`{REPO_TABLE}`) | **반드시 값으로 치환**되거나, 조건부 섹션 규칙에 따라 그 섹션과 함께 통째로 제거된다 | `{REPO_TABLE}` |
| **B. 저작 지시** (template-only) | `<!-- TEMPLATE-ONLY -->` … `<!-- /TEMPLATE-ONLY -->` 로 감싼 블록 | **블록 전체를 마커째 삭제**한다. 인스턴스에 남으면 결함 | "이 섹션은 …일 때만 렌더한다", 변수 목록·채움 출처 설명 |
| **C. 인스턴스 잔존 안내** | 마커 없는 평문 | 그대로 남는다 | 필드 정의, 접근 규율, 시크릿 경계 |

**(2) C(잔존 안내) 안에서는 `{UPPER_SNAKE}`를 쓰지 않는다 — 하드 규칙**

**따름 규칙 — 충전 슬롯은 백틱으로 감싸지 않는다.** 그래야 "백틱 안의 `{UPPER_SNAKE}` = 슬롯 이름을 *가리키는* 산문"이라는 판별이 **예외 없이** 성립하고, 생성기가 *채울 자리*와 *언급*을 기계적으로 가를 수 있다(백틱 슬롯이 하나라도 있으면 그 판별이 무너진다 — 실증 확인). 그리고 인스턴스에 남을 산문이 충전 슬롯이나 환경변수·설정 키를 가리켜야 하면 **중괄호 없는 맨 이름**으로 쓴다 (`` `REPO_TABLE` ``·`` `WORKSPACE_ROOT` ``) 또는 사람이 읽는 섹션명으로 부른다("레포 인벤토리 표"). 중괄호를 씌우는 순간 그 문장은 (1)-A와 형태가 같아져 **판정 불가능**해지고, 잔존 검사가 거짓 양성을 낸다. 슬롯 이름을 나열해야 하는 설명은 C가 아니라 **B(TEMPLATE-ONLY)** 에 넣는다.

> **예시 자리표는 허용된다.** `{repo-slug}`·`{lang}`·`{예: 2026-01-cycle}`처럼 **소문자·하이픈·공백·비ASCII가 섞인** 중괄호 표기는 "여기에 이런 값이 온다"는 *예시*이며 (1)-A가 아니다. 인스턴스에 남아도 정상이다 — 나중에 사람·다른 에이전트가 그 자리를 채울 때 읽는 형식 안내다. 형태가 겹치지 않으므로 기계 판정이 갈린다.

**(2-1) 템플릿에 *룰북 표기 변수*를 쓰지 않는다 — 하드 규칙 (예시 자리표 허용과 혼동 금지)**

에이전트 룰북은 자기 본문에서 `{메타 레포}`·`{공유리포}`·`{Q2.2}` 같은 **룰북 표기 변수**로 좌표를 가리킨다(그 문서 안에서는 정상이다). 그러나 **템플릿 본문에는 이 표기를 쓰지 않는다.** 이유는 두 겹이다:

- **의미가 다르다** — 이것은 "여기에 이런 값이 온다"는 *예시*가 아니라 **실제 값으로 치환돼야 하는 좌표**다. 치환되지 않으면 인스턴스에 리터럴로 남아 독자에게 *채우다 만 자리*로 보인다.
- **기계가 못 잡는다** — 비ASCII·공백이 섞여 (1)-A `{UPPER_SNAKE}` 패턴에 걸리지 않으므로 **잔존 검사(3)가 조용히 통과시킨다.** 예시 자리표와 형태가 같아 판별이 무너지는, 위 (2)가 막으려던 바로 그 상황이다.

**따라서 템플릿에서 좌표를 가리켜야 하면 (2)와 같은 처리를 한다** — 중괄호 없는 맨 이름(`메타 레포 루트의 .env`)이나 사람이 읽는 이름으로 쓰거나, 실제로 값을 채워야 하는 자리라면 정식 `{UPPER_SNAKE}` 충전 슬롯으로 선언한다. (실측된 결함: 어느 템플릿이 참조 맵에 `{메타 레포}/.env`를 적어 인스턴스에 그대로 남았고, `[PLH]` 정규식이 대문자만 보므로 통과했다 — 다른 템플릿은 같은 상황을 "중괄호 없는 맨 이름으로 쓴다"고 이미 규정하고 있었다.)

**(3) 잔존 검사 판정 규칙 (생성 후 — 실행자가 지어내지 않는다)**

생성된 **인스턴스**(템플릿 원본이 아니다)에 대해:

| 판정 | 조건 |
|---|---|
| **FAIL — 미충전 슬롯 잔존** | `{UPPER_SNAKE}` 패턴이 하나라도 남아 있다. 정규식 `\{[A-Z][A-Z0-9_]*\}`, 단 바로 앞 문자가 `$`인 것(`${VAR}` — 셸·CI 변수 참조)은 제외 |
| **FAIL — 저작 지시 전사** | `TEMPLATE-ONLY` 문자열이 남아 있다 (B 블록을 지우지 않았다는 증거) |
| **PASS — 정상 중괄호** | 위 둘에 걸리지 않는 모든 중괄호: 예시 자리표(소문자·비ASCII 혼용), 코드·설정 리터럴(JSON `{}`·`${VAR}`·정규식) |

OS 중립 판정 명령(정본 — 인스턴스 파일 경로들을 인자로):

```bash
python -c "import sys;sys.stdout.reconfigure(encoding='utf-8', errors='backslashreplace');import re;[ [print('FAIL',p,i+1,l.rstrip()) for i,l in enumerate(open(p,encoding='utf-8').read().splitlines()) if re.search(r'(?<!\$)\{[A-Z][A-Z0-9_]*\}',l) or 'TEMPLATE-ONLY' in l] for p in sys.argv[1:]] or print('PASS')" ORCHESTRATOR.md REPO-MAP.md
```

> 출력이 비면(=`PASS`만) 통과. `python`이 없는 환경이면 동치 판정을 다른 수단으로 하되, **판정에 쓴 명령과 그 실측 출력을 보고에 싣는다** (POLICY-VERIFY — "봤다"는 클레임은 증거가 아니다).
