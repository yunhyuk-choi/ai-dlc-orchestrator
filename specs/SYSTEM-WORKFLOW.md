...
```bash
python -c "import re,sys;[ [print('FAIL',p,i+1,l.rstrip()) for i,l in enumerate(open(p,encoding='utf-8').read().splitlines()) if re.search(r'(?<!\$)\{[A-Z][A-Z0-9_]*\}',l) or 'TEMPLATE-ONLY' in l] for p in sys.argv[1:]] or print('PASS')" ORCHESTRATOR.md REPO-MAP.md
```
