...
6.8. **(조건부) 하네스 온보딩 안내** (S5.9 인터뷰 + S9.5) — 프레임워크를 *바깥에서 구동*하는 선택적 외부 시스템(하네스)을 쓸 것인지 인터뷰에서 수집하고(S5.9), **팀 최초 설치자면** 검증 이후·종료 이전에 그 하네스 온보딩으로 안내한 뒤 산출된 대시보드 좌표를 인스턴스에 기록한다(S9.5). 목록은 `specs/HARNESS-CATALOG.md`, **온보딩 지식은 각 하네스 레포가 소유** — SETTER는 실행하지 않고 카탈로그 행을 그대로 전달한다. 하네스 미사용이면 스킵 (조건부)
...
| 하네스 온보딩 절차·config 스키마·설치 실행 | **각 하네스 레포** (SETTER는 `specs/HARNESS-CATALOG.md` 행을 *안내만*) |
...
#### S9.5. 하네스 온보딩 안내 (최초 설치자만) — **조건부**

> *왜 여기인가 (배치 근거)*: ① 하네스 온보딩은 **`dlc-meta` 공유 원격 URL이 확정·검증된 상태**를 입력으로 받는다 — 그 검증이 끝나는 곳이 S8.6(push)·S9(체크)다. ② S9 뒤라는 것은 **하네스 온보딩이 실패해도 코어 부트스트랩은 이미 성사**됐다는 뜻이다 — 실패가 부트스트랩으로 번지지 않게 격리된다 (EX-15와 같은 사상: **C FALLBACK**, 중단 금지).
> *목록·상세*: `{공유리포}/specs/HARNESS-CATALOG.md`. **온보딩 지식은 각 하네스 레포가 소유** — SETTER는 실행하지 않고 카탈로그 행을 *그대로 전달*한다(절차를 복제하면 하네스가 바뀔 때마다 이 리포에 PR이 필요해진다 — 원칙 8 마찰).
...
| **없음** | 아직 아무도 안 붙였다 → **최초 설치자** | 아래 수행 |

**수행 (안내다 — 실행이 아니다)**

1. 카탈로그에서 선택된 행을 읽어 **레포 URL · 진입 방법 · 전제조건 · 최초 설치자 경로**를 *그대로* 사용자에게 전달한다 (해석·요약·복제 금지).
2. 온보딩에 넘길 입력을 함께 준다: **검증된 `dlc-meta` 공유 원격 URL**(Q2.4 — S8.6 push·S9 검증 완료). 하네스는 이 좌표로 시스템 인스턴스에 닿는다.
3. **대시보드 좌표 기록 (합류자를 위한 결정적 단계)** — 온보딩이 대시보드(관리 UI) URL을 산출하면, `{메타 레포}/ORCHESTRATOR.md`의 **「부착된 하네스」 섹션**(템플릿 조건부 섹션 — S7에서 제거해 뒀다)을 추가하고 그 값을 기록한 뒤 `dlc-meta`에 **커밋·push**한다 (POLICY-TEMPLATE-ADHERENCE·POLICY-TRACKING, 쓰기는 POLICY-ENCODING). 이 기록이 성사돼야 **합류자가 clone만으로 좌표를 손에 넣는다** — 입력도 안내도 불필요해진다. 기록이 없으면 합류자는 좌표를 못 받고, 다음 사람이 다시 "최초"로 판정된다.
   - 온보딩이 *이 세션 밖*(다른 클론·다른 사람·나중)에서 끝나면 **같은 자리에 기록하도록** 좌표(파일 경로·섹션명)를 명시해 전달한다.
   - **토큰·시크릿 값은 기록하지 않는다** (자격증명 경계 / POLICY-TRACKING).
4. 보고에 결과를 한 줄로 남긴다 — 안내함 / 성사·기록함 / 미성사(사유).

**실패 처리 (C FALLBACK — 중단 금지)** — 전제조건 미충족·온보딩 실패·사용자 이탈 어느 경우든 **부트스트랩을 실패로 만들지 않는다.** 이미 S9까지 성사됐다. 사유를 보고에 남기고 S10으로 간다. **ABORT·PAUSE로 격상 금지**, 사용자 컨펌을 기다리며 멈추지도 않는다(사람 없는 자동 기동 세션이 정상 케이스).

> *하네스 부착은 부트스트랩에 묶이지 않는다* — 나중에 누구든 카탈로그 행의 진입 방법을 직접 실행해 붙일 수 있다. S9.5는 *마침 부트스트랩 중이니 알려주는* 자리일 뿐이다.
