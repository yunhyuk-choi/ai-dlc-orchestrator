...
| Q5.9.3 | 그 행의 **전제조건**을 충족하는가? (미충족이면 지금 붙이지 않는다 — 나중에 카탈로그 보고 직접 붙일 수 있다) | 조기 차단 (헛수고 방지) |
...
4. **디스패치 패키지 조립** — 아래 6개만. S9 보고 「권고 다음 단계」에 실어 반환한다 (컨텍스트 격리 = 최소 정보 — `ORCHESTRATOR-AGENT.md` §3.2):
...
> *넘기지 않는 것*: 인터뷰 응답 전량, 프레임워크 내부 룰북, 시크릿 값. **절차 설명도 넣지 않는다** — 룰북이 갖고 있다(넣으면 그 순간 두 원천이 갈라진다).
